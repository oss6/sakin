...
var smartGeneration = utils.chooseOption(program.enableSmart, program.disableSmart, settings['smart_generation']);
...
if (program.init) {
    ...
    writer.createProject(action, { theme: program.theme || 'default' }, function (errors) {
        if (errors.length !== 0) {
    ...
    fs.stat('.sakin', function (err) {
        if (err && err.code === 'ENOENT') {
            console.log(theme.error('Not a sakin workspace. Type \'sakin -i\' to create a site.'));
            process.exit(1);
        }
    });
}
