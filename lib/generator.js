...
x.createIndex = function (g, cb) { // articles
    reader.read('articles', 'all', function (articles, errors) {
        articles = articles.map(function (article) {
            ...
            var urlParts = ['blog', date.year, date.month, date.day, slug(article.metadata.title, {lower: true})];
        ...
        articles.sort(function (a1, a2) {
            return utils.compareDates(a1.date, a2.date);
        });
...
x.processFiles = function (action, pages, articles, cb) {
    ...
    x.createIndex(g, function (res, errors) {
        ...
        writer.saveToOutput(action, processed, function (saveErrors) {
            ...
            // Commit changes
            var date = new Date();
            contentsRepo.commit('Contents updated at ' + date.toISOString());
...
x.generate = function (action, smart, cb) {
    ...
    x.getModifiedFiles(contentsRepo, function (err, pageFiles, articleFiles, deletedFiles) {
        ...
        reader.read('pages', smart ? pageFiles : 'all', function (pages, pErrors) {
            if (pErrors.length !== 0) {
                errors = errors.concat(pErrors);
            }

            reader.read('articles', smart ? articleFiles : 'all', function (articles, aErrors) {
                if (aErrors.length !== 0) {
...
x.watchChanges = function (action, directory) {
    ...
    watcher.on('error', function () {
        action.log('error', 'An error occurred while watching changes in the contents directory');
    });
