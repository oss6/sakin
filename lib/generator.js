...
x.processFiles = function (action, sakinFile, pages, articles, cb) {
    ...
    pages.forEach(page => {
        ...
        sakinFile.contents[page.metadata.path] = _path;
    ...
    articles.forEach(article => {
        ...
        // Update mappings (to save later in .sakin file)
        sakinFile.contents[article.metadata.path] = _path;
        sakinFile.categories = x.updateCategoriesMapping({
            mode: 'addDelete',
            mapping: sakinFile.categories,
            articleUrl: _url,
    ...
    // --------------------------------------------------
    g.type = 'category';
    var processedCategories = utils.flatten(articles.map(article => article.metadata.categories));
    var categories = Object.keys(sakinFile.categories).filter(category => processedCategories.indexOf(category) > -1);
