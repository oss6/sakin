...
x.createIndex = function (g, sakinFile, customContent, cb) {
    if (g['custom_index']) {
        ...
        // Generate menu
        var dirLevel = './';
        g.menuItems = x.createMenu(g.menu, 'index', dirLevel, sakinFile);
...
x.createMenu = function (menuMapping, srcTitle, dirLevel, sakinFile) {
    var menuItems = [];
    ...
    Object.keys(menuMapping).forEach(name => {
        ...
        var destTitle = mappingObj.filename || mappingObj.url;
        var href;

        // Get href attribute
        if (srcTitle === destTitle) {
            href = 'index.html';
        } else if (contentsPath.split('/')[0] !== 'contents') {
...
x.processFiles = function (action, sakinFile, pages, articles, cb) {
    ...
    pages.forEach(page => {
        ...
        // Generate menu
        g.menuItems = x.createMenu(g.menu, slug(page.metadata.title, {lower: true}), dirLevel, sakinFile);
    ...
    articles.forEach(article => {
        ...
        // Generate menu
        g.menuItems = x.createMenu(g.menu, slug(article.metadata.title, {lower: true}), dirLevel, sakinFile);
    ...
    Object.keys(sakinFile.categories).forEach(category => {
        ...
        // Generate menu
        g.menuItems = x.createMenu(g.menu, category, dirLevel, sakinFile);
